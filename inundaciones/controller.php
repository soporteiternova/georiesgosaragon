...
class controller {
    ...
    /**
     * Gets flood list from opendata repository.
     * @return bool
     */
    protected function crondaemon($debug = false) {
        ...
        if ( $debug || $datetime === '0025' ) {
            ini_set( 'memory_limit', '4G' );
            $count = 0;
            $api_url = \georiesgosaragon\common\controller::get_endpoint_url( \georiesgosaragon\common\controller::ENDPOINT_INUNDACIONES );
            $array_objs = json_decode( file_get_contents( $api_url ) );

            if ( isset( $array_objs->features  )) {
                foreach ( $array_objs->features as $obj ) {
                    $obj_model = new model();
                    $obj_model->update_from_api( $obj );
                    $count++;
                }
            }
            echo '<br/><br/><br/><br/><br/>Updated ' . $count . ' floods';
        }
